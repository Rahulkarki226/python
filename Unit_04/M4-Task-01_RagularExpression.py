...
import re
txt="This is a sample uth string. It contains words that era start with vowels."
x=re.findall('\\b[aeiouAEIOU]\S+',txt)
print(x)
...
x=re.search("([0-1][0-2]\.[0-1][0-2]\.[0-9]{4})",txt)
print(x.group())
...
import re
txt="ihgiur jgj hAggj &jsgsgh"
x=re.findall("\\b[a-z|A-Z|0-9]\S*",txt)
print(x)

#Q6. write a python program to search a string which start with y or Y or z or Z
import re
txt="klmojmjod jgjge Zkggj yjsgsgh"
print(re.findall("\\b[yYzZ]\S*",txt))

import re
# The regular expression to match the pattern
pattern = re.compile(r'a[A-Za-z0-9]')

# Test strings
test_strings = ['abc', 'a1', 'aB', 'Aa', '1a']
...
import re
txt="klmojmjod jgj Zkggj +jsgsgh"
print(re.findall("\\b[A-Za-z]{3}\\b",txt))